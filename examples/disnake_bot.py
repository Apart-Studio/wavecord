...
"""Same idea as basic.py, but on disnake instead of discord.py.

The only difference from the discord.py version is the import: WaveCord is
library-agnostic, so the rest of the code is identical.

Run:
  pip install "disnake>=2.9"
  export DISCORD_TOKEN=your-bot-token # a Lavalink node must be running
  python examples/disnake_bot.py
"""
